...
setTimeout(() => {
  ...
  var vclog = function (msg) {
    console.log(`%c[SB VC] ${msg}`, "color: #768bd1");
  };
  ...
  navigator.mediaDevices
    ...
    .catch((err) => {
      vclog(
        "Mic permission denied. Please enable it in your browser settings."
      );
      alert(
        "Mic permission denied. Please enable it in your browser settings."
      );
  ...
  function calcVolumes(listenerPos, soundPos) {
    ...
    if (
      listenerPos.x == "unknown" ||
      listenerPos.y == "unknown" ||
      soundPos.x == "unknown" ||
      soundPos.y == "unknown"
    )
      return [0, 0];
  ...
  const sendPos = throttle(
    (id, x, y) => socket.emit("pos", id, getGame(), x, y),
    25
  );
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (shiftKey && altKey && !event.repeat) {
      toggleAudioStream();
    }
  });
  ...
  function initPeer() {
    ...
    peer.on("open", (id) => {
      vclog(`My peer ID is: ${id}`);
    });
    peer.on("disconnected", (id) => {
      vclog("lost connection");
    });
    peer.on("error", (err) => {
      console.error(err);
    });
  ...
  function ingamecheck() {
    if (window.location.pathname == "/") {
      if (
        Object.values(window.module.exports.settings).find((v) => v && v.mode)
          .mode.id != "welcome"
      ) {
  ...
  setInterval(() => {
    ...
    if (
      Object.values(
        Object.values(window.module.exports.settings).find((v) => v.mode)
      ).find((v) => v.status).status != undefined &&
      ingamecheck()
    ) {
      window.myPos = Object.values(
        Object.values(window.module.exports.settings).find((v) => v.mode)
