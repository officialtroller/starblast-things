...
var gameOptions = {
    ...
    issue: [`⚠️ Main code has been edited ⚠️`, `Issues might occur in the future`],
    ...
    */
        spectator: {
            // not more then one spectator ship.
            191: {
                name: 'Spectating',
        ...
        admin: {
            // admin ships are specific, you must have and advanced amout of skill in coding in order to add more.
            192: {
        ...
        main: {
            // add pretty much anything you want.
            101: { name: 'Fly', code: `` },
            102: {
                name: 'Gnat',
    ...
    images: {
        ...
        ModVersion: {
            ...
            positions: { x: 21, y: -8.5, width: 22, height: 9, depth: -0.3 },
        },
    },
    vocabulary: [
        // 1
...
function getShipAFKinfo(ship) {
    return new Promise(resolve => {
        ...
        const check_3 = function () {
            const isAFK = ship.custom.r1 === ship.custom.r2 || (ship.custom.point1.x === ship.custom.point2.x && ship.custom.point2.x === ship.x && ship.custom.point1.x === ship.x && ship.custom.point1.y === ship.custom.point2.y && ship.custom.point2.y === ship.y && ship.custom.point1.y === ship.y && ship.custom.r1 === ship.custom.r2);
            resolve(isAFK);
        };
...
function findColor(ship, KDr) {
    switch (true) {
        case !ship.alive:
            return 'rgb(55, 55, 55)';
        case ship.custom.isAFK:
            return 'rgb(200, 111, 111)';
        case ship.custom.spectator:
            return 'rgb(155, 155, 155)';
        case ship.name === gameOptions.Your_in_game_NAME:
            return gameOptions.Your_in_game_COLOR;
        case ship.name.includes(gameOptions.Creator):
            return '#005cb9';
        case ship.id === KDr[0].id && ship.custom.Kills >= 1:
...
function updateScoreboard(game) {
    ...
    const Scoreboard = {
        ...
        clickable: false,
        visible: true,
        components: [{ type: 'box', position: [0, 0, 100, 9.75], fill: 'rgba(255, 255, 255, 0.35)' }, { type: 'box', position: [81, 0, 7.5, 9.75], fill: 'rgba(55, 255, 55, 0.35)' }, { type: 'box', position: [88.5, 0, 7.5, 9.75], fill: 'rgba(255, 55, 55, 0.35)' }, { type: 'text', position: [3, 0.5, 69, 8], value: 'Players', color: 'rgba(255,255,255,0.8)', align: 'left' }, { type: 'text', position: [65.75, 0.8, 29, 8], value: 'K/D', color: 'rgba(255,255,255,0.8)', align: 'right' }, ...players.flat()],
    };
...
function getCords(size, info, random = true) {
    ...
    const newInfo = Array(mapSize)
        .fill(0)
        .map((_, i) => info.cords - Math.round(size / 2) + i);
...
function spectator_ship(ship, rps = true) {
    if (!ship.custom.spectator_switch || game.step >= ship.custom.spectator_switch) {
        ...
        if (gameOptions.adminShip.includes(ship.type.toString())) {
            ...
            ship.set({
                type: gameOptions.spectatorShip[0],
                crystals: 0,
                stats: 0,
                shield: 999,
                collider: false,
...
function admin_ship(ship, off = false) {
    ...
    if (ship.type.toString() === gameOptions.spectatorShip[0]) spectator_ship(ship);
        ...
        ship.set({
            type: next_type,
            collider: collider,
            stats: gameOptions.adminShip.includes(next_type.toString()) ? 0 : 11111111 * Math.trunc(next_type / 100),
...
function respawnShip(ship) {
    ...
    if (gameOptions.BannedList.includes(ship.name.replace(/[\[\]]/g, '|'))) {
        setTimeout(() => {
            alert(ship, `Warning!`, 'Your name matches with a banned player name.', 'rgba(255,55,55,0.8)');
        }, 3500);
    }
}
    ...
    switch (event.name) {
        case 'ui_component_clicked':
            if (event.id === 'using_subspace' && gameOptions.Enable_antiCheat) {
                ...
                switch (event.id) {
                    case 'Menu_':
                        update_Menu(event.ship);
                        break;
                    case 'HideShow_Buttons':
                        Manage_Buttons(event.ship);
                        break;
                    case 'Regen':
                        regen(event.ship);
                        break;
                    case 'Spectate':
                        ...
                        break;
                    case 'Admin':
                        admin_ship(event.ship);
                        break;
                    case 'Next':
                        next_ship_button(event.ship);
                        break;
                    case 'Previous':
                        previous_ship_button(event.ship);
                        break;
                    case 'Stats':
                        Stats_button(event.ship);
                        break;
                    case 'Center':
                        Teleport_Center(event.ship);
                        break;
                    case 'Warp':
                        warp_ship(event.ship, game);
                        break;
                    case 'Info':
                        moreEvent(event.ship);
                        break;
                }
            }
            break;
...
game.modding.commands.info = function () {
    ...
    let playerList = 'List of Players and their IDs:\n';
    for (let i = 0; i < totalPlayers; i++) {
        const player = game.ships[i];
        playerList += `Index: ${i}, Name: ${getPlayerName(player)}, Ship type: ${player.type}, Coordinates: X: ${Math.round(player.x)}, Y: ${Math.round(player.y)}\n`;
    }
    game.modding.terminal.echo(`!bot ${playerList}`);
    game.modding.terminal.echo('\n');
};
...
idle = function (who, showMessage = true) {
    ...
    game.modding.terminal.echo(`!bot Player: ${getPlayerName(ship)} (${who}), has been frozen.`);
};
...
unidle = function (who) {
    ...
    game.modding.terminal.echo(`!bot Player: ${getPlayerName(ship)} (${who}), has been unfrozen.`);
};
...
kick = function (who, reason = 'Disturbing duels') {
    ...
    if (!game.ships.includes(ship)) {
        ...
        game.modding.terminal.echo(`!bot The index you used doesn't exist, try again with a valid index`);
        return;
    }
    game.ships.forEach(function (ship) {
        alert(ship, `Player: ${getPlayerName(ship)} has been kicked`, '', 'rgba(255,155,55,0.8)');
    });
    idle(who, false);
    ...
    game.modding.terminal.echo(`!bot Player: ${getPlayerName(ship)}, index: ${who}, has been kicked`);
};
...
ban = function (who, reason = 'Disturbing duels') {
    ...
    game.modding.terminal.echo(`!bot Player: ${getPlayerName(ship)}, index: ${who}, has been banned`);
    for (const otherShip of game.ships) {
        ...
        if (otherShip.name.replace(/[\[\]]/g, '|') == ship.name.replace(/[\[\]]/g, '|')) {
            setTimeout(() => {
                alert(otherShip, `Warning!`, 'Your name matches with a banned player name.', 'rgba(255,55,55,0.8)');
            }, 3500);
        }
    }
};
...
game.modding.commands.banlist = function () {
    if (!gameOptions.BannedList.length > 0) {
        ...
        game.modding.terminal.echo(`!bot There are no banned players in this game.`);
        return;
    }
    let bannedpl1ers = 'Banned players:\n';
    game.modding.terminal.echo(`[[g;#70aeff;]\nBanned player list:]\n\n[[g;#689bdd;]Banned Players Amount: ${gameOptions.BannedList.length}]\n`);
    gameOptions.BannedList.forEach((player, index) => {
        game.modding.terminal.echo(`[[g;#70e4ff;]Index: ${index}, Name: ${player}, Reason: ${gameOptions.BannedListReasons[index]}]`);
    }, (bannedpl1ers += `Index: ${index}, Name: ${player}, Reason: ${gameOptions.BannedListReasons[index]}\n`));
    game.modding.terminal.echo(`\n[[gi;#ffa770;]INFO] [[gi;#e2ff70;]Type] [[gbi;#70ccff;]unban(index)] [[gi;#e2ff70;]to unban a player.]`);
    game.modding.terminal.echo(`!bot ${bannedpl1ers}`);
};

unban = function (index) {
    if (!gameOptions.BannedList.includes(gameOptions.BannedList[index])) {
        game.modding.terminal.error(new Error("You gave a wrong index or the player that you're trying to unban isn't banned or already got unbanned."));
        game.modding.terminal.echo(`!bot You gave a wrong index or the player that you're trying to unban isn't banned or already got unbanned.`);
        return;
    }
    game.ships.forEach(ship => {
        alert(ship, `Player: ${gameOptions.BannedList[index]} has been unbanned.`, '', 'rgba(55,255,55,0.8)');
    });
    game.modding.terminal.echo(`[[g;#70ffc1;]\nPlayer: ${gameOptions.BannedList[index]}, reason: ${gameOptions.BannedListReasons[index]} , Has successfully been unbanned]`);
    game.modding.terminal.echo(`!bot Player: ${gameOptions.BannedList[index]}, reason: ${gameOptions.BannedListReasons[index]} , Has successfully been unbanned`);
...
admin = function (who, duration = Infinity) {
    ...
    if (ship.custom.admin) {
        ...
        game.modding.terminal.echo(`!bot Player: ${getPlayerName(ship)}, index: ${who} has had their admin commands removed`);
    } else {
        if (duration !== Infinity) {
            ship.custom.adm = setTimeout(() => {
                ...
                game.modding.terminal.echo(`!bot Player: ${getPlayerName(ship)}, index: ${who} has lost their admin powers`);
            }, duration * 1000);
        }
        ...
        game.modding.terminal.echo(`!bot Player: ${getPlayerName(ship)}, index: ${who}, Duration: ${duration} has been given the admin commands`);
    }
};
...
game.modding.commands.apc = function () {
    ...
    game.modding.terminal.echo(`!bot Always Pickup Crystals feature is now ${status}`);
};

set = function (who, what, max_crystals = 0, max_stats = 0) {
    const ship = game.ships[who];
    if (ship.custom.isAFK) {
        game.modding.terminal.error(new Error(`${getPlayerName(ship)} is AFK and cannot be switched to another ship.`));
        game.modding.terminal.echo(`!bot ${getPlayerName(ship)} is AFK and cannot be switched to another ship.`);
        return;
    }
    ...
    game.modding.terminal.echo(`!bot Player ${getPlayerName(ship)} (index: ${who}) has been given:\nShip Type: ${what}, Crystals: ${max_crystals}, Stats: ${max_stats}`);
};
...
say = function (text = '', duration = 4) {
    ...
    game.modding.terminal.echo(`[[g;#70ffc1;]\nText: ${text} applied]`);
    game.modding.terminal.echo(`!bot Text: ${text} applied`);
};
